...
在due框架中，路由被设计成了无状态（stateless）和有状态（stateful）两种模式。两种路由模式分别对应着分布式集群中不同的路由分发机制。但无论是哪种路由模式，一个路由号只能对应一种路由模式。
...
![无状态路由分发](../images/route-stateless-dispatch.png)
...
![有状态路由定向转发](../images/route-stateful-forward.png)
