...
1. 通过环境变量指定，例如： DUE_ETC=/run/etc
2. 通过启动参数指定，例如： --etc=./etc
...
[https://pkg.go.dev/github.com/dobyte/due/v2/etc](https://pkg.go.dev/github.com/dobyte/due/v2/etc)

## 2.4.6 示例代码

以下完整示例详见：[etc-example](https://github.com/dobyte/due-docs/tree/master/examples/etc-example)
...
文件位置：[etc-example/etc/db.toml](https://github.com/dobyte/due-docs/blob/master/examples/etc-example/etc/db.toml)
...
文件位置：[etc-example/main.go](https://github.com/dobyte/due-docs/blob/master/examples/etc-example/main.go)
...
以下配置[etc.toml](https://github.com/dobyte/due/blob/main/testdata/etc/etc.toml)为框架所有模块的详细配置，你可以根据自己模块使用情况进行选择性配置。
